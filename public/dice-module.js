...
let dieList = null;
let debugPhysicsToggle = null;
let apiBase = '/api';
let includeTemplateDownload = true;
let tickStarted = false;
let rollPayloadFactory = () => ({});
let onRollFinished = null;
...
async function createDie(sides) {
  const response = await fetch(`${apiBase}/dice`, {
    method: 'POST',
...
async function rollDie(dieId) {
  const response = await fetch(`${apiBase}/dice/${dieId}/roll`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  if (onRollFinished) onRollFinished({ dieId, roll: data.roll });
  return data;
}
...
export function initDiceModule(options = {}) {
  dieList = options.dieList;
  debugPhysicsToggle = options.debugPhysicsToggle || null;
  apiBase = options.apiBase || '/api';
  includeTemplateDownload = options.includeTemplateDownload ?? true;
  rollPayloadFactory = typeof options.rollPayloadFactory === 'function'
    ? options.rollPayloadFactory
    : () => ({ tuning: { ...debugTuning } });
  onRollFinished = typeof options.onRollFinished === 'function' ? options.onRollFinished : null;

  if (!dieList) throw new Error('initDiceModule requires dieList element');
