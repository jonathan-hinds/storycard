...
export class PhaseManagerClient {
  ...
  buildTemplateFromMatch(currentMatch) {
    ...
    currentMatch.players[OPPONENT_SIDE].board.forEach((card, index) => {
      ...
      initialCards.push({
        ...
        focalMarkBonusDamage: Number.isFinite(Number(card.focalMarkBonusDamage))
          ? Math.max(0, Math.floor(Number(card.focalMarkBonusDamage)))
          : 0,
  ...
  syncCardBuffStateFromMatch(currentMatch) {
    ...
    this.client.cards.forEach((sceneCard) => {
      ...
      sceneCard.userData.focalMarkBonusDamage = state?.focalMarkBonusDamage ?? 0;
