...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/phase-manager/matchmaking/find') {
    ...
    sendJson(res, 200, await phaseManagerServer.findMatch(body.playerId, {
      ...
      solo: body.solo === true,
      npcProfile: body.npcProfile && typeof body.npcProfile === 'object' ? body.npcProfile : null,
    }));
    return true;
  }
  ...
  if (req.method === 'POST' && pathname === '/api/phase-manager/match/commit-animation-complete') {
    let body = {};
    try {
      body = await readRequestJson(req);
    } catch (error) {
    ...
    const result = phaseManagerServer.completeCommitAnimations(body);
    if (result.error) {
      sendJson(res, result.statusCode || 400, { error: result.error });
      return true;
    }
  ...
  if (req.method === 'POST' && pathname === '/api/phase-manager/match/spell/start') {
    let body = {};
    try {
      body = await readRequestJson(req);
    } catch (error) {
    ...
    const result = phaseManagerServer.startSpellResolution(body);
    if (result.error) {
      sendJson(res, result.statusCode || 400, { error: result.error });
      return true;
    }
  ...
  if (req.method === 'POST' && pathname === '/api/phase-manager/match/spell/roll') {
    let body = {};
    try {
      body = await readRequestJson(req);
    } catch (error) {
    ...
    const result = phaseManagerServer.submitSpellRoll(body);
    if (result.error) {
      sendJson(res, result.statusCode || 400, { error: result.error });
      return true;
    }
  ...
  if (req.method === 'POST' && pathname === '/api/phase-manager/match/spell/complete') {
    let body = {};
    try {
      body = await readRequestJson(req);
    } catch (error) {
    ...
    const result = phaseManagerServer.completeSpellResolution(body);
    if (result.error) {
      sendJson(res, result.statusCode || 400, { error: result.error });
      return true;
    }
